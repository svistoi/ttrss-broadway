defmodule DownloadTest do
  ...
  test "redirect article" do
    ...
    {:ok, temp3} = Temp.path()
    {:ok, temp4} = Temp.path()
    Task.async(fn -> DownloadTranscode.download_http_poison("https://cbc.mc.tritondigital.com/CBC_WR_P/media/wr/wr-6mGl5gAK-20200513.mp3", temp1) end)
    |> Task.await(300_000)
    Task.async(fn -> DownloadTranscode.download_http_poison("https://cbc.mc.tritondigital.com/CBC_WR_P/media/wr/wr-6mGl5gAK-20200513.mp3", temp2) end)
    |> Task.await(300_000)
    Task.async(fn -> DownloadTranscode.download_http_poison("https://22163.mc.tritondigital.com:443/CBC_WR_P/media-session/60342a3f-9042-499f-8a1a-5d446bb5af16/wr/wr-AjYYQx5o-20200513.mp3", temp3) end)
    |> Task.await(300_000)
    Task.async(fn -> DownloadTranscode.download_http_poison("https://podcast-a.akamaihd.net/wr/wr_20200513.mp3", temp4) end)
    |> Task.await(300_000)
    File.rm(temp1)
    File.rm(temp2)
    File.rm(temp3)
